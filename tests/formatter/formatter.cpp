...
TEST_CASE("Testing Formatter - Basic Types") {
    // 文字列連結
    std::string result1 = nodec::Formatter() << "Hello" << " " << "World";
    CHECK(result1 == "Hello World");

    // 整数
    std::string result2 = nodec::Formatter() << "Number: " << 42;
    CHECK(result2 == "Number: 42");

    // 浮動小数点
    std::string result3 = nodec::Formatter() << "Float: " << 3.14;
    CHECK(result3 == "Float: 3.14");

    // 文字
    std::string result4 = nodec::Formatter() << 'A' << 'B' << 'C';
    CHECK(result4 == "ABC");
}
