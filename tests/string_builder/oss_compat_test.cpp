...
TEST_CASE("StringBuilderStream - Enhanced Manipulator Support") {
    // setw テスト
    std::string buffer1;
    nodec::StringBuilderStream stream1(buffer1);
    stream1 << std::setw(10) << 42;
    CHECK(buffer1 == "        42"); // 右揃え、幅10

    // setfill テスト
    std::string buffer2;
    nodec::StringBuilderStream stream2(buffer2);
    stream2 << std::setfill('0') << std::setw(5) << 123;
    CHECK(buffer2 == "00123");

    // setprecision テスト
    std::string buffer3;
    nodec::StringBuilderStream stream3(buffer3);
    stream3 << std::fixed << std::setprecision(2) << 3.14159;
    CHECK(buffer3 == "3.14");

    // 組み合わせテスト
    std::string buffer4;
    nodec::StringBuilderStream stream4(buffer4);
    stream4 << std::setfill('*') << std::setw(8) << "Hi";
    CHECK(buffer4 == "******Hi");
}

TEST_CASE("StringBuilderStream - Number Base Support") {
    // 16進数テスト（小文字）
    std::string buffer1;
    nodec::StringBuilderStream stream1(buffer1);
    stream1 << std::hex << 255;
    ...
    // 8進数 with showbase
    std::string buffer5;
    nodec::StringBuilderStream stream5(buffer5);
    stream5 << std::oct << std::showbase << 64;
    CHECK(buffer5 == "0100");

    // 10進数に戻す
    std::string buffer6;
    nodec::StringBuilderStream stream6(buffer6);
    stream6 << std::hex << 16 << ", " << std::dec << 16;
    CHECK(buffer6 == "10, 16");
}

TEST_CASE("StringBuilderStream - Alignment Support") {
    // 左揃え
    std::string buffer1;
    nodec::StringBuilderStream stream1(buffer1);
...
TEST_CASE("StringBuilderStream - Float Formatting") {
    // fixed フォーマット
    std::string buffer1;
    nodec::StringBuilderStream stream1(buffer1);
...
TEST_CASE("StringBuilderStream - ostringstream Compatibility Check") {
...
TEST_CASE("StringBuilderStream - Complex Format Chains") {
    // 複雑なフォーマットチェーン
    std::string buffer;
    nodec::StringBuilderStream stream(buffer);
